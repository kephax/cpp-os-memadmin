...
//////////////////////////////////////////////////////////////////////
void	Application::vergeetEigen(int m)
{
	require(! objecten.empty());    // hebben we eigenlijk wel wat ?
	...
	if (m > 0) {
		ALiterator  i;
		for (i = objecten.begin() ; (m > 0) && (i != objecten.end()) ; --m) {
			ap = *i;                            // MYCODE
            i = objecten.erase(i);              // MYCODE
		}
	}
	else {
		objecten.pop_front();
...
/* //////////////////////////////////////////////////////////////////////////////////////////////////// */
void Application::points(double time)
{
    float nPoints = size * time;
    std::string str = "";
    cout << " Size (" << size << ") * Time (" << time << ") = " << nPoints << endl;

    if( (nPoints >     0) && (nPoints <  1000) ) { str = "A"; } else
    if( (nPoints >  1000) && (nPoints <  2000) ) { str = "B"; } else
    if( (nPoints >  2000) && (nPoints <  3000) ) { str = "C"; } else
    if( (nPoints >  3000) && (nPoints <  4000) ) { str = "D"; } else
    if( (nPoints >  4000) && (nPoints <  5000) ) { str = "E"; } else
    if( (nPoints >  5000) && (nPoints <  6000) ) { str = "F"; } else
    if( (nPoints >  6000) && (nPoints <  7000) ) { str = "G"; } else
    if( (nPoints >  7000) && (nPoints <  8000) ) { str = "H"; } else
    if( (nPoints >  8000) && (nPoints <  9000) ) { str = "I"; } else
    if( (nPoints >  9000) && (nPoints < 10000) ) { str = "J"; } else
    if( (nPoints > 10000) && (nPoints < 11000) ) { str = "K"; } else
    if( (nPoints > 11000) && (nPoints < 12000) ) { str = "L"; } else
    if( (nPoints > 12000) && (nPoints < 13000) ) { str = "M"; } else
    if( (nPoints > 13000))                       { str = "Z"; }

    cout << "Total Points (Size/Time): " << str <<  endl;
}


/* //////////////////////////////////////////////////////////////////////////////////////////////////// */
void	Application::eigenscenario(int aantal, bool vflag)
{
    bool old_vflag = this->vflag;
    this->vflag = vflag;	// verbose mode aan/uit
    ...
    for (int  x = 0 ; x < aantal ; ++x)  	// Doe nu tig-keer "iets".
    {
        ...
        if (objecten.empty()				// Als we nog niets hebben of
                || vraagkans(r) )			// we kiezen voor ruimte aanvragen
        {
            // De omvang van de geheugen wordt random bepaald maar zal klein zijn
            // om onze eigen Java applicatie te simuleren.
            r = randint(2,10);              // MYCODE
			vraagGeheugen(r);               // MYCODE
        }
        else								// Anders: vrijgeven mits ...
            if (!objecten.empty())  		// ... we iets hebben
            {
                vergeetEigen(randint(0,objecten.size()));
            }
        // else
        // dan doen we een keer niets
    }
